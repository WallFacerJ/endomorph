...
- **Business context is surfaced everywhere an entity appears; whether it should also weight the score is an open question.** `packages/fabric` emits asset criticality, rationale, and business unit keyed by entity id; it rides on the scenario as an optional `assets` block, and every surface now reads it -- the Endpoint and Identity consoles badge and triage by it, SIEM flags the results that touch a severe- or high-criticality asset, and the case evidence graph marks its nodes. The "generated but unread" gap is closed for presentation. What is deliberately not done: weighting the coverage score or a finding by criticality. An incident's ground-truth entities are already the ones the intrusion touched, so a low-criticality host on the attack path matters as much as a severe one that was never involved; weighting coverage by business criticality could reward reaching the important-looking entity over the actually-relevant one. Left as a decision rather than an oversight.
