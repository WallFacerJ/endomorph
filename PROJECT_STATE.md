...
- **Business context is surfaced on all three telemetry consoles, not yet in Case.** `packages/fabric` emits asset criticality, rationale, and business unit keyed by entity id; it rides on the scenario as an optional `assets` block, and the Endpoint, Identity, and SIEM consoles all read it -- the entity consoles badge and triage by it, and SIEM flags the results that touch a severe- or high-criticality asset. The remaining gap is Case: it does not yet weight a finding, or the coverage score, by the criticality of what the analyst reached, so scoping a domain controller counts the same as scoping a print-room workstation.
