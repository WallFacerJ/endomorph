...
2. **Publish a stable benchmark corpus.** *Generation done:* `pnpm benchmark`
   ...
   "the Endomorph Detection Benchmark v1.0: 12 intrusions, 41 techniques, 56.6k
...
3. **Grow ATT&CK coverage deliberately.** Technique breadth is the number
   detection engineers judge a corpus on. Twelve plans now, spanning through the Impact tactic (ransomware) with mail/phishing, cloud, DNS, and web/proxy domains; each new
   plan is pure event modelling, no infrastructure. (The benchmark manifest now
   makes the number, 41 techniques, visible and diffable.)
4. **Make FP realism a measured headline.** *Done:* `pnpm noise-floor` reports,
   ...
   false-positive floor for an unspecific rule. At the shipped seed 34/41
